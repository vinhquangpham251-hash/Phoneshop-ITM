...
export const Header = () => {
	...
	const handleSearch = (e) => {
		e.preventDefault();
		if (searchQuery.trim()) {
			history.push(`/tim-kiem?q=${encodeURIComponent(searchQuery.trim())}`);
		}
	};
						...
						{currentUser ? (
							<Dropdown className="nav-item no-arrow" align="end">
								...
								<Dropdown.Menu className="animated--grow-in">
									...
									{currentUser?.userGroup?.name === 'ADMIN' && (
										<>
											<Dropdown.Divider />
											<Dropdown.Item as={NavLink} to="/admin">
												<i className="fas fa-fw fa-sm fa-tachometer-alt text-gray-400 me-2"></i>
